document.addEventListener("DOMContentLoaded", () => {
    ...
    // dentro do popup (ícones, preview e campos)
    ...
    // elementos do painel de bytes no popup
    ...
    let currentIndex = -1;
    let currentDraft = null; // draft que espelha o formulário e que é serializado para bytes
    ...
    // ---------- Buffer / LocalStorage helpers (compatível com create.js) ----------
    function loadBuffer() {
        const raw = localStorage.getItem(STORAGE_KEY);
        if (!raw) return new Uint8Array(2); // header mínimo
        try {
    ...
    // ---------- Construção / leitura de registros no novo formato ----------
    // Monta um record (Uint8Array) a partir do objeto produto (mantém lapide e id)
    function produtoToRecordBytes(produto) {
        ...
        const recordLen = 1 + 2 + sizeData; // lapide + tamanhoRegistro + dados
    ...
    // Decodifica um record a partir do buffer começando em startOffset
    // Retorna { produto, startOffset, nextOffset, recordBytes } ou null se inválido
    function decodeRecordAt(buffer, startOffset) {
        const total = buffer.length;
        if (startOffset >= total) return null;
        if (startOffset + 3 > total) return null; // lapide + tamanho (1+2)
    ...
    // Percorre todo o buffer e retorna lista de {produto, startOffset, nextOffset, recordBytes}
    function parseAllRecords() {
        const buffer = loadBuffer();
        const records = [];
        if (!buffer || buffer.length <= 2) return records;
        let off = 2; // pula header ultimoId
    ...
    // monta a lista de produtos ativos (apenas objetos produto)
    ...
    // abre popup por índice na lista atual de produtos
    function abrirPopupByIndex(i) {
        ...
        // atualiza painel de bytes a partir do draft
    ...
    // Atualiza painel de bytes com a serialização do draft (REAÇÃO)
    function updateByteViewFromDraft(draft) {
        if (!draft) return;
        const record = produtoToRecordBytes(draft); // Uint8Array
        const uint8 = record;
        ...
        // escreve no DOM (segura existência dos elementos)
    ...
    // ---------- interactions: ícones + inputs reativos ----------
    iconOptions.forEach(opt => {
    ...
    // ---------- salvar alterações: reserializa e substitui no buffer principal ----------
    if (form) {
        form.addEventListener("submit", (e) => {
            e.preventDefault();
            if (!currentDraft) return;

            // garante valores mais recentes
            currentDraft.nomeProduto = editNome.value;
            currentDraft.gtin = editGtin.value;
            currentDraft.descricao = editDescricao.value;
            ...
            // procura registro pelo id e obtém offsets
            const recs = parseAllRecords();
            let found = null;
            for (const r of recs) {
            ...
            if (!found) {
                ...
                // mantemos header (ultimoId) intacto
            }

            // salva
            saveBuffer(novoBuffer);

            // atualiza UI: reconstrói lista e fecha popup
    ...
    // handlers de fechamento do popup
    ...
    // pesquisa (opera sobre lista construída)
    if (searchInput) {
